...
function playMusic(type='battle'){
  ...
  if(!_reverb&&audioCtx){
    try{
      ...
      _reverb.buffer=buf;
      const rg=audioCtx.createGain(); rg.gain.value=0.15;
      _reverb.connect(rg); rg.connect(musicGain);
    }catch(e){}
  }
  if(type==='menu') scheduleMenuMusic();
  else scheduleBattleMusic();
}
...
function str(freq, start, dur, vol){
  ...
  try{
    const harmonics=[[1,1],[2,0.45],[3,0.22],[4,0.1],[5,0.05]];
